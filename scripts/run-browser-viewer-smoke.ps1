...
try {
    ...
    Write-Host "Waiting a few seconds to confirm both processes stay alive..."
    if (-not (Wait-ForStartupHealth -PublisherProcess $publisherProcess -ViewerProcess $viewerProcess -TimeoutS 5)) {
        throw "publisher or viewer exited during startup."
    }
    ...
    while ($true) {
        ...
        if ($publisherProcess.HasExited) {
            if ($browserOpened) {
                ...
                Write-Host "Publisher completed without a browser connection."
                $exitCode = 1
            }
            break
        }
        ...
        if ($viewerProcess.HasExited) {
            $viewerExitCode = $viewerProcess.ExitCode
