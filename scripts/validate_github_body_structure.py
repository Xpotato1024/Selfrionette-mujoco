...
def _inline_heading_fragment_lines(lines: list[str]) -> list[str]:
    """Return outside-fence lines containing two or more heading-like fragments."""
    fragments: list[str] = []
    opening: tuple[str, int] | None = None
    for line in lines:
        match = FENCE_RE.fullmatch(line)
        if match:
            ...
            if opening and marker_type == opening[0] and marker_length >= opening[1] and not rest.strip():
                opening = None
                continue
        if opening is None and len(re.findall(r"(?:^|\s)#{1,6}\s+\S", line)) >= 2:
...
def validate(
    ...
    *,
    ...
    heading_like_count = len(re.findall(r"(?:^|\s)#{1,6}\s+\S", before_lf))
