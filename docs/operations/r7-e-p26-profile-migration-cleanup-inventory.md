...
### Issue #392 implementation-result note

この追記はinventory時点のbaseline/classificationを変更しない。

- **P26-VIEWER-003:** PoC retirement remains complete. The invalid fixture regeneration reproduced on both current `main` and the #392 branch; it was not a PoC dependency or a reason to restore the retired package.
- **P26-VIEWER-004:** the single canonical owner remains `apps/mujoco-viewer/public/fixtures/fast_arm_sweep_x_qpos.json`. The rejected candidate `A30FD0A303506C7807BA2E687411FACDF28BA2BC2AE9AC8F909B9C59997FEE36` was not accepted. The repaired current path generated 30 frames with schema version 1, native MuJoCo source, the fast_arm scene path, qpos dimension 4, consecutive indices, strictly increasing time, finite qpos, intended sweep progression, and no BADQACC warning. Final SHA-256: `4925D77535A67ED0E4EB68BDCC0B66C262D2D11AE5E1F7DCA99C3AE5E38D312A`.
- **Root-cause fix:** direct qpos application now clears stale MuJoCo velocity before `mj_forward`/`mj_step`; `sweep_x` move and return frames carry their interpolated desired endpoint. The exporter validates the complete sequence and atomically replaces the product fixture only after validation and serialization succeed.
- **Migrated assertions and evidence:** product viewer fixture tests now cover schema/source/model/preset, frame count, qpos dimension and finiteness, consecutive indices, strictly increasing time, and meaningful progression. The real native dry-run path covers qpos application, MuJoCo stepping, warning counters, stable object identity, intentional terminal hold, and deterministic exporter output. Historical #178/#181/#183/#184/#185 and the historical docs remain traceable.
- **Behavior-preservation result:** runtime payload schema, Viewer Profile compatibility, product route/startup contract, native MuJoCo physical-state ownership, IK/FK ownership, and P25 pacing/coalescing were preserved. No hardware or external side effect was used.
