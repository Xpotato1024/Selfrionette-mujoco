...
有効な同条件の検証は再利用し、不完全レビューは不足部分を補完する。環境障害は失敗箇所から再開する。全面実操作E2Eはリリース前の統合段階、変更に必要な検証はその場で行う。関連Skillから古い担当指定・追加の全面再実行義務を持ち込まない。条件・独立性・権限境界の正本は[実行ワークフロー](docs/operations/codex-workflow.md)。
