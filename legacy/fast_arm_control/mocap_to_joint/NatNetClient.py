...
class NatNetClient:
    def __init__( self, local_ip="127.0.0.1", server_ip="127.0.0.1", multicast_ip="239.255.42.99" ):
        ...
        self.multicastAddress = multicast_ip
    ...
    def __createCommandSocket( self ):
        ...
        result.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        result.bind( (self.localIPAddress, self.commandPort) )
    ...
    def __unpackSkeleton( self, data ):
        ...
        for j in range( 0, rigidBodyCount ):
            add_offset, _, _, _ = self.__unpackRigidBody( data[offset:] )
            offset += add_offset
    ...
    def __unpackMocapData( self, data ):
        ...
        for i in range( 0, rigidBodyCount ):
            add_offset, id, pos, rot =  self.__unpackRigidBody( data[offset:] )
            rigid_bodies.append({"id": id, "position": pos, "rotation": rot})
            offset += add_offset
        ...
        if self.rigidBodyListener is not None:
            self.rigidBodyListener(frameNumber, timestamp, rigid_bodies)
    ...
    def __dataThreadFunction( self, socket, lock, name ):
    ...
    def run( self ):
        ...
        dataThread = Thread( target = self.__dataThreadFunction, args = (self.dataSocket, self.end_flag_lock, "data"))
        dataThread.start()

        # Create a separate thread for receiving command packets
        commandThread = Thread( target = self.__dataThreadFunction, args = (self.commandSocket, self.end_flag_lock, "comm"))
        commandThread.start()
